...
candidates:
  ...
  - id: C17
    ...
    strength: strong
    status: done
    adr: docs/adr/0001-layering-contracts-live-in-pyproject.md
---
...
**Status: done** — merged as `3bcd1b0`. Brief [briefs/C13-one-client-host-contract.md](briefs/C13-one-client-host-contract.md).
...
**Status: done** — merged as `cf28d61`. Brief [briefs/C14-client-wiring-seam.md](briefs/C14-client-wiring-seam.md).
...
**Status: done** — implemented as `16e9796`, no brief: the one open design question was
settled in session (`JSONDict`/`JSONList` → `aiochainscan/types.py`, `coerce_response_items` →
`aiochainscan/domain/response.py`), and the rest was mechanical. The decision is recorded in
[../adr/0001-layering-contracts-live-in-pyproject.md](../adr/0001-layering-contracts-live-in-pyproject.md).
The silent-skip branch in `preflight.sh` / `validate_fast.sh` (`[ -f .importlinter ]` → green
"skipped" line) was found during implementation and fixed in the same commit — deleting the config
file without it would have disabled the check while still reporting success.
