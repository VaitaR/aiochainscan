...
> **Outcome (2026-09-13):** all eight candidates implemented and merged to local main as
> merge commit `5abd365` (branch `refactor/arch-c23-c30`), one commit per candidate:
> C23 `b4be6fe`, C24 `16c3c86`, C26 `822fd4f`, C30 `4fc0410`, C25 `fb6dc1e`,
> C27 `ca18d55`, C28 `28821ca`, C29 `f60764a`. No briefs were promoted — implemented
> directly from this file. Documented deviations from the section text: C24 declared the
> previously-implicit `'sepolia': 'sepolia'` alias pass-through (byte-identical behaviour,
> required for true single-sourcing); C25 unified the provider label on `scanner/network`
> (two tests re-pinned); C26 drops one inert probe row (phantom `scanner_version='v9'`).
> `make validate` green: 2115 passed, 0 failed.
