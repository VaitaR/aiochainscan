...
async def main():
    ...
    for (name, version), scanner_class in available_scanners.items():
        ...
        print(f"   📦 {name} {version}: {method_count} methods, networks: {networks}, auth: {auth_info}")
    ...
    if len(results) >= 2:
        ...
        for method, balance in results:
            if isinstance(balance, str) and balance.isdigit() or isinstance(balance, int | str) and str(balance).isdigit():
