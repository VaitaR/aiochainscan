...
class SimpleRateLimiter(RateLimiter):
    ...
    async def acquire(self, key: str = 'default') -> None:
        if self._min_interval <= 0.0:
            return
        # The lock is keyed with the same scope as the timing state.  This
        # serializes competing acquires for one key while leaving other keys
        # independent.
        ...
        async with lock:
            ...
            regen = int(elapsed / self._min_interval)
