...
class TestGetBlockParamContract:
    async def test_etherscan_block_by_number_maps_block_number_to_tag(self) -> None:
        ...
        fake = FakeNetwork(response={'result': {'number': '0x1'}})
    ...
    async def test_blockscout_block_by_number_substitutes_path_param(self) -> None:
        fake = FakeNetwork(response={'hash': '0xabc', 'height': 19_500_000})
...
class TestPreviouslyOrphanEtherscanEndpoints:
    ...
    async def test_tx_status_check(self) -> None:
        fake = FakeNetwork(response={'result': {'isError': '0'}})
    ...
    async def test_block_countdown(self) -> None:
        fake = FakeNetwork(response={'result': {'EstimateTimeInSec': '120'}})
    ...
    async def test_block_number_by_timestamp(self) -> None:
        fake = FakeNetwork(response={'result': '12345'})
        await _etherscan(fake).call(
    ...
    async def test_gas_estimate(self) -> None:
        fake = FakeNetwork(response={'result': '120'})
    ...
    async def test_proxy_get_balance(self) -> None:
        fake = FakeNetwork(response={'result': '0xde0b6b3a7640000'})
...
def _client_with_fake_network(fake: FakeNetwork) -> Any:
    """Real ChainscanClient wired to the fake network via the constructor seam.

    The scanner is built by the constructor as usual (``scanner=None``) and
    receives the injected ``network`` as its ``network_client`` per the
    constructor's contract.
    """
    from aiochainscan.chain_registry import resolve_scanner_target
    from aiochainscan.core.client import ChainscanClient

    target = resolve_scanner_target('etherscan', 'main', api_key='test_key')
    return ChainscanClient(target, network=fake)


class TestClientConvenienceThroughSeam:
    async def test_get_block_countdown_convenience_uses_blockno_wire_param(self) -> None:
        fake = FakeNetwork(response={'status': '1', 'result': {'EstimateTimeInSec': '120'}})
    ...
    async def test_check_transaction_status_convenience(self) -> None:
        fake = FakeNetwork(response={'result': {'isError': '0', 'errDescription': ''}})
        client = _client_with_fake_network(fake)
    ...
    async def test_get_contract_creation_convenience(self) -> None:
        fake = FakeNetwork(
            response={'status': '1', 'result': [{'contractAddress': '0xC', 'txHash': '0xT'}]}
        )
        client = _client_with_fake_network(fake)
    ...
    async def test_get_erc721_transfers_convenience(self) -> None:
        fake = FakeNetwork(
            response={'status': '1', 'message': 'OK', 'result': [{'tokenID': '42'}]}
        )
        client = _client_with_fake_network(fake)
    ...
    async def test_get_gas_estimate_convenience(self) -> None:
        fake = FakeNetwork(response={'status': '1', 'result': '120'})
        client = _client_with_fake_network(fake)
    ...
    async def test_eth_get_balance_convenience(self) -> None:
        fake = FakeNetwork(response={'jsonrpc': '2.0', 'result': '0xde0b6b3a7640000'})
        client = _client_with_fake_network(fake)
    ...
    async def test_unknown_tx_hash_reads_as_none_not_a_transient_error(self) -> None:
        ...
        fake = FakeNetwork(response=None)
        client = _client_with_fake_network(fake)
